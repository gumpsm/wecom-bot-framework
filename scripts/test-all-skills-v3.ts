...
async function main() {
  ...
  if (schedId) {
    try { await call(client, "schedule", "update_schedule", { schedule: { schedule_id: schedId, summary: "[P2 updated] skill test schedule", attendees: [{ userid: "ShiMeng" }] } }); pass("schedule.update_schedule"); } catch (e) { fail("schedule.update_schedule", (e as Error).message); }
    await sleep(200);
  ...
  try {
    await call(client, "schedule", "check_availability", { check_user_list: ["ShiMeng"], start_time: daysLater(1) + " 09:00:00", end_time: daysLater(1) + " 18:00:00" });
  ...
  if (docId) {
    try { await call(client, "doc", "get_doc_content", { type: 2, docid: docId }); pass("doc.get_doc_content"); } catch (e) { fail("doc.get_doc_content", (e as Error).message); }
    await sleep(500);
  } else { skip("doc.get_doc_content", "no doc"); }

  // edit_doc_content (needs content_type)
  if (docId) {
    try { await call(client, "doc", "edit_doc_content", { content_type: 1, content: "# P2 Test\n\nAutomated skill test content.", docid: docId }); pass("doc.edit_doc_content"); } catch (e) { fail("doc.edit_doc_content", (e as Error).message); }
    await sleep(500);
