...
//
// 环境变量设计（v0.4.0 统一格式）：
//   Bot 凭据:   WECOM_{BOT_NAME}_ID / _BOT_SECRET
...
async function loadBotConfigs(
  ...
  if (botName) {
    ...
    var botId = env["WECOM_" + prefix + "_ID"] || rawConfig.botId || "";
    var botSecret = env["WECOM_" + prefix + "_SECRET"] || rawConfig.botSecret || "";
  ...
  // 多 Bot 模式：扫描 bots/ 目录
  // BOT_FILTER: 逗号分隔 bot 名，只启动指定的
  var botFilter = env.BOT_FILTER ? env.BOT_FILTER.split(",").map(s => s.trim()) : null;
  var botDirs = fs
  ...
  for (var di = 0; di < botDirs.length; di++) {
    var dir = botDirs[di];
    // BOT_FILTER filter
    if (botFilter && botFilter.indexOf(dir.name) < 0) {
    ...
    try {
      ...
      var botId2 = env["WECOM_" + prefix2 + "_ID"] || rawCfg.botId || "";
      var botSecret2 = env["WECOM_" + prefix2 + "_SECRET"] || rawCfg.botSecret || "";

      if (!botId2 || !botSecret2) {
        console.warn("[warn] 跳过 " + dir.name + ": .env 中缺少 WECOM_" + prefix2 + "_ID / _SECRET");
        continue;
      }
      ...
      permissions: rawCfg.permissions || undefined,
...
async function main() {
  ...
  // enable MCP
  var mcpBotId = env.WECOM_PA_BOT_ID;
  var mcpBotSecret = env.WECOM_PA_BOT_SECRET;
  if (mcpBotId && mcpBotSecret) {
    await botManager.enableMcp({ botId: mcpBotId, botSecret: mcpBotSecret });
  } else {
    console.warn("MCP disabled - no credentials");
  }
