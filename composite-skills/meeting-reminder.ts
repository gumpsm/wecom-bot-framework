...
export var meetingReminderDefinition: SkillDefinition = {
  ...
  description: "会前提醒。在会议开始前指定分钟数发送提醒消息到群。自动计算提醒时间并注册到 cron-scheduler",
  parameters: {
    type: "object",
    properties: {
      meetingTitle: { type: "string", description: "会议主题" },
      meetingTime: { type: "string", description: "会议时间，格式：YYYY-MM-DD HH:MM" },
      remindBefore: { type: "string", description: "提前多少分钟提醒，默15" },
      chatId: { type: "string", description: "提醒发送的chatId" },
...
export async function setMeetingReminder(
  ...
  var notifyText = "📢 会议提醒\n" +
    "「" + meetingTitle + "」在" + meetingTime + " 开始（" + remindBefore + "分钟后）\n" +
    "参会" + (attendees || "全员") + "\n" +
    "请准时参加！";
