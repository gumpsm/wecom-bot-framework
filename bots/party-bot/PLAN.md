...
- **流程**：确认类型 → 收集信息 → LLM 生成 → `doc_create_doc` + `doc_edit_doc_content`
- **验收**：用户说「写一份主题党日活动新闻稿」，Bot 追问活动详情后生成文档并返回链接
- **依赖 Skill**：`doc_create_doc`、`doc_edit_doc_content`
...
### 场景 4：活动/会议日程（P1）
- **触发**：「安排党员大会」「主题党日活动」
- **流程**：确认时间/参与人 → `schedule_create_schedule`
- **依赖 Skill**：`schedule_create_schedule`（已有）
