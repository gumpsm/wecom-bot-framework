...
### 1.8 一次一个变更（Simplicity First / Surgical Changes）
- 每次只修复一个问题，不把多个不相干的改动混在一次提交中
- 改完立即验证，验证通过再改下一个
- 不添加需求之外的功能，不为"万一以后用得上"做过度设计
- 不重构没坏的东西，不顺手"优化"相邻代码、注释或格式
- 判断标准：diff 的每一行都应该能直接追溯到用户的一个具体需求
- 如果 200 行能写成 50 行，重写它

### 1.9 目标驱动验证（Goal-Driven Execution）
- 修 Bug → 先写复现测试 → 修复 → 测试通过
- 新增功能 → 先定义验收标准 → 实现 → 验证
- "修复了"不够，必须有可验证的成功标准
- 对于 framework 改动：单元测试 + 集成测试（pa-bot 真实企业微信操作）都必须通过
- 强有力的成功标准让 AI 能独立循环验证；弱标准（"让它工作"）需要反复人工确认

### 1.10 不确定时停下来（Think Before Coding）
- 遇到歧义不猜测，主动列出假设让 PO/用户选择
- 存在多种理解时列出所有可能，而不是悄悄选一个
- 发现自己不理解某段代码时不"顺手改"，先讨论
- 有更简单方案时主动提出，适时反驳不合理需求
- 困惑时立刻停下来，指明不清楚的地方，请求澄清
- ❌ 在欢迎信息或文档中使用硬编码的可变数据（应从源文件动态读取）
